...
namespace Term
{
	...
	*/
	...
	/** Split a string and store each new substring in a deque. */
...
{
	string result = "";
	// split the strings, create deques
	...
	if (pad)
	{
		// find the largest line out of any of the deques
		int max = 0;
		...
		// for each line in both deques, pad the line for max size
		for (int i = 0; i < lv.size(); i++)
		{
			if (lv[i].size() < max)
			{
	...
	// loop through the deques and produce the new string
	for (int i = 0; i < lv.size(); i++)
	{
		result += lv[i] + rv[i] + '\n';
	}
...
std::string Term::fuse(std::initializer_list<string> strings, bool pad)
{
	...
	string result = "";
	// Moving the initializer_list into a deque b/c I think they're easier
 ...
 * filled a deque with strings ....
 *
 * @param strings a deque of strings ordered left to right
 *
 * @param pad bool, whether to pad each line of the string to be the same width
 */
 ...
 * returned substrings
 * @return deque containing each substring
 */
...
std::deque<std::string> Term::rsplit(string text, string delim, bool include)
{
	...
	if (include)
	{
		...
		while (iter != end)
		{
			...
			// of the NEXT string in the deque
...
Term::IO &Term::IO::operator<<(string text)
{
	...
	for (int i = 0; i < strings.size(); i++)
	{
		...
		if (wide)
		{
			...
			// the first string in the deque (which can't contain a color code
